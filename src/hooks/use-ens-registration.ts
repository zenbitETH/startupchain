...
import { getOwner, getPrice, getResolver } from '@ensdomains/ensjs/public'
...
export function useEnsRegistration() {
  ...
  const checkNameRegistration = useCallback(
    async (
      name: string
    ): Promise<{ isRegistered: boolean; owner?: string }> => {
      ...
      try {
        ...
        try {
          const ownerResult = await getOwner(publicClient, {
            name: normalizedName,
          })
          ...
          if (
            ownerResult &&
            ownerResult.owner &&
            ownerResult.owner !== '0x0000000000000000000000000000000000000000'
          ) {
            console.log('✅ Name is registered to owner:', ownerResult.owner)

            // Try to get resolver too
            try {
              const resolverResult = await getResolver(publicClient, {
                name: normalizedName,
              })
    ...
    async (name: string): Promise<boolean> => {
      ...
      try {
        ...
        try {
          ...
          if (
            err.message?.includes('NameNotAvailable') ||
  ...
  const getRegistrationCost = useCallback(
    async (
      name: string,
      durationYears: number
    ): Promise<{ costEth: string; costWei: bigint }> => {
      if (!publicClient) throw new Error('Public client not available')
  ...
  const checkWalletBalance = useCallback(async (): Promise<{
    balance: string
    hasEnough: boolean
    cost: bigint
  }> => {
  ...
  const makeCommitment = useCallback(
    async (params: ENSRegistrationParams): Promise<string> => {
      ...
      if (!isValidEnsName(params.name)) {
        throw new Error(
          'Invalid ENS name format. Name must be 3-63 characters, contain only letters, numbers, and hyphens, and not start/end with hyphens.'
        )
      }
      ...
      try {
        ...
        const { costEth, costWei } = await getRegistrationCost(
          params.name,
          params.durationYears
        )
        console.log(
          '💰 Actual registration cost for',
          params.name + ':',
          costEth,
          'ETH'
        )
        ...
        console.log(
          '💰 Checking embedded wallet balance (this wallet pays for ENS registration)...'
        )
        ...
        if (!hasEnough) {
          throw new Error(
            `Insufficient balance. You need ${costEth} ETH but only have ${balance} ETH. Get Sepolia ETH from: https://sepoliafaucet.com/`
          )
        }
        ...
        console.log(
          '🎯 Registering to address:',
          ownerAddress,
          params.owner ? '(custom address)' : '(wallet address)'
        )
        ...
        console.log(
          `✅ ENS name "${normalizedName}" - proceeding with commitment`
        )
        ...
        console.log(
          '🔐 Creating commit transaction with params:',
          registrationParams
        )
        ...
        console.log(
          '🔗 View commit transaction: https://sepolia.etherscan.io/tx/' +
            txHash
        )
        ...
        console.log(
          '⏳ Commitment transaction submitted, not waiting for confirmation to avoid blocking UI'
        )
        ...
        localStorage.setItem(
          'ens-commitment-' + normalizedName,
          JSON.stringify(commitmentData)
        )
        console.log(
          '💾 Commitment stored in localStorage as backup:',
          commitmentData
        )
        ...
        const message =
          err instanceof Error ? err.message : 'Failed to make commitment'
  ...
  const register = useCallback(
    async (params: ENSRegistrationParams): Promise<string> => {
      ...
      console.log(
        '  - Time since commit:',
        commitment ? Date.now() - commitment.timestamp : 'N/A'
      )
      ...
      if (!activeCommitment) {
        ...
        if (stored) {
          try {
            ...
            console.log(
              '✅ Recovered commitment from localStorage:',
              activeCommitment
            )
      ...
      if (!activeCommitment) {
        console.error(
          '❌ REGISTRATION FAILED: No commitment found in state OR localStorage!'
        )
      ...
      if (timeSinceCommit < 60 * 1000) {
        throw new Error(
          `Please wait ${Math.ceil((60 * 1000 - timeSinceCommit) / 1000)} more seconds before registering`
        )
      }
      ...
      try {
        ...
        console.log(
          '📝 Creating registration transaction with params:',
          registerParams
        )
        ...
        console.log(
          '🔗 View registration transaction: https://sepolia.etherscan.io/tx/' +
            txHash
        )
        ...
        // Don't wait for confirmation to avoid blocking UI
        console.log(
          '⏳ Registration transaction submitted, not waiting for confirmation to avoid blocking UI'
        )
        console.log('🔗 Transaction will be confirmed in background')

        console.log(
          `✅ ${activeCommitment.name} registered to ${activeCommitment.owner}`
        )

        // Wait a moment then check if registration actually worked
        console.log(
          '🔍 Waiting 10 seconds then checking registration status...'
        )
        setTimeout(async () => {
          try {
            ...
            if (registrationStatus.isRegistered) {
              console.log(
                '✅ VERIFICATION SUCCESS: Name is registered to:',
                registrationStatus.owner
              )
              console.log(
                '🎉 You can now check your name at: https://sepolia.app.ens.domains/' +
                  (activeCommitment?.name || params.name + '.eth')
              )
            } else {
              console.log(
                '❌ VERIFICATION FAILED: Name is not showing as registered'
              )
              console.log('🔗 Check transactions manually:')
              console.log(
                '  - Commitment: https://sepolia.etherscan.io/tx/' +
                  (activeCommitment?.hash || 'unknown')
              )
              console.log(
                '  - Registration: https://sepolia.etherscan.io/tx/' + txHash
              )
            }
          } catch (verifyError) {
        ...
        const message =
          err instanceof Error ? err.message : 'Failed to register ENS name'
        console.error('❌ Error message:', message)
        setError(message)
        throw err
      } finally {
  ...
  return {
    ...
    debugCurrentState: () => {
      ...
      console.log(
        '  - Can Register:',
        !!commitment && Date.now() - commitment.timestamp >= 60 * 1000
      )
